...
class Y1HelperApp(tk.Tk):
    ...
    def setup_ui(self):
        ...
        # Auto-update system frame - moved up to make room for release notes
        update_frame = ttk.Frame(main_frame)
        update_frame.pack(fill=tk.X, pady=(2, 0))  # Reduced top padding
        
        # Update status label
        self.update_status_var = tk.StringVar(value="Checking for updates...")
        self.update_status_label = ttk.Label(update_frame, textvariable=self.update_status_var,
                                           relief="flat", borderwidth=1, padding=(4, 2),
                                           font=("Segoe UI", 8))
        self.update_status_label.pack(fill=tk.X, side=tk.LEFT, expand=True)
        
        # Install update button (hidden by default)
        self.install_update_btn = ttk.Button(update_frame, text="Install Update", 
                                           command=self.install_update, style="TButton")
        self.install_update_btn.pack(side=tk.RIGHT, padx=(5, 0))
        self.install_update_btn.pack_forget()  # Hidden by default
        
        # Release notes frame - new area for displaying latest release notes
        self.release_notes_frame = ttk.Frame(main_frame)
        self.release_notes_frame.pack(fill=tk.X, pady=(1, 0))
        
        # Release notes text widget for multi-line display with wrapping
        self.release_notes_text = tk.Text(self.release_notes_frame, 
        ...
        self.release_notes_text.pack(fill=tk.X, side=tk.LEFT, expand=True)
        
        # Initially hide release notes
        self.release_notes_frame.pack_forget()
        ...
        # GitHub tokens for auto-update system
        self.github_tokens = [
            "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",
            "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",
            "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",
            "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",
            "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",
            "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",
            "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"
        ]
        self.current_token_index = 0
        
        # Initialize update-related attributes
        self.latest_release = None
        
        # Start auto-update check with a delay to avoid blocking UI startup
    ...
    def check_for_updates(self):
        """Check for updates using GitHub API with token rotation"""
        import requests
        import json
        import random
        ...
        def check_update():
            try:
                ...
                # Get latest release
                url = "https://api.github.com/repos/team-slide/Y1-helper/releases/latest"
                ...
                if response.status_code == 200:
                    ...
                    if self.compare_versions(latest_version, self.version) > 0:
                        # Update available
                        self.update_status_var.set(f"Y1 Helper v{latest_version} now available")
                        self.install_update_btn.pack(side=tk.RIGHT, padx=(5, 0))
                        
                        # Store update info
                        self.latest_release = release_data
                        debug_print(f"Update available: v{latest_version}")
                        
                        # Show release notes preview
                        self.show_release_notes_preview(release_data)
                    else:
                        self.update_status_var.set(f"Y1 Helper v{self.version} is up to date")
                        # Clear any stored release data when no update is available
                        if hasattr(self, 'latest_release'):
                            delattr(self, 'latest_release')
                        # Hide release notes when no update is available
                        self.hide_release_notes_preview()
                        debug_print("No update available")
                else:
                    self.update_status_var.set("Update check failed")
                    debug_print(f"Update check failed: {response.status_code}")
                    
            except Exception as e:
                self.update_status_var.set("Update check failed")
                debug_print(f"Update check error: {e}")
        
        # Run in background thread
        import threading
        threading.Thread(target=check_update, daemon=True).start()
    
    def show_release_notes_preview(self, release_data):
        """Show a preview of the latest release notes"""
        try:
            release_notes = release_data.get('body', 'No release notes available')
            if release_notes and release_notes.strip():
                # Clean up the preview (remove markdown formatting)
                import re
                preview = re.sub(r'#+\s*', '', release_notes)  # Remove headers
                ...
                self.release_notes_text.insert(1.0, f"Latest: {preview}")
                self.release_notes_text.config(state=tk.DISABLED)
                
                self.release_notes_frame.pack(fill=tk.X, pady=(1, 0))
                debug_print("Release notes preview shown")
            else:
                self.hide_release_notes_preview()
        except Exception as e:
            debug_print(f"Error showing release notes preview: {e}")
            self.hide_release_notes_preview()
    
    def hide_release_notes_preview(self):
        """Hide the release notes preview"""
        try:
            self.release_notes_frame.pack_forget()
            self.release_notes_text.config(state=tk.NORMAL)
            self.release_notes_text.delete(1.0, tk.END)
            self.release_notes_text.config(state=tk.DISABLED)
            debug_print("Release notes preview hidden")
        except Exception as e:
            debug_print(f"Error hiding release notes preview: {e}")
    ...
    def install_update(self):
        ...
        # Double-check that this is actually an update
        if not self.latest_release or 'tag_name' not in self.latest_release:
            messagebox.showerror("Error", "Invalid update information")
            return
            
        latest_version = self.latest_release['tag_name'].lstrip('v')
        if self.compare_versions(latest_version, self.version) <= 0:
            messagebox.showerror("Error", "No update available - you already have the latest version")
            return
        
        try:
            # Find the executable asset dynamically
            exe_asset = None
            if 'assets' in self.latest_release and self.latest_release['assets']:
                for asset in self.latest_release['assets']:
                    if asset and 'name' in asset and asset['name'].endswith('.exe'):
                        exe_asset = asset
                        break
            ...
            # Progress label
            progress_label = ttk.Label(progress_window, text="Downloading update...", font=("Segoe UI", 10))
            ...
            # Status label
            status_label = ttk.Label(progress_window, text="", font=("Segoe UI", 8))
            ...
            def download_update():
                try:
                    ...
                    # Update status
                    self.update_status_var.set("Downloading update...")
                    ...
                    for chunk in response.iter_content(chunk_size=chunk_size):
                        if chunk:
                            ...
                            if total_size > 0:
                                ...
                                progress_label.config(text=f"Downloading update... {progress:.1f}%")
                                status_label.config(text=f"{downloaded / 1024 / 1024:.1f} MB / {total_size / 1024 / 1024:.1f} MB")
                            else:
                                progress_label.config(text=f"Downloading update... {downloaded / 1024 / 1024:.1f} MB")
                    ...
                    # Hide release notes preview before updating
                    self.hide_release_notes_preview()
                    
                    # Update status
                    self.update_status_var.set("Launching update installer...")
                    
                    try:
                        # Run the update executable
                        import subprocess
                        subprocess.Popen([temp_file.name], creationflags=subprocess.CREATE_NEW_CONSOLE)
                        
                        # Close this application immediately
                        self.quit()
            ...
            messagebox.showerror("Error", f"Failed to start update: {e}")
            self.update_status_var.set("Update failed")
            self.install_update_btn.config(state="normal")
